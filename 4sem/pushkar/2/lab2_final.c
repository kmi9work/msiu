...
void gnuplot_input(double x_start, double x_end, int *method){
  ...
  if (method[1] == YES) {
    ...
    fprintf(file,"'method_Runge_Kutt_2' title \"Runge-Kutta methods of order 2\" with lines");
  }
  if (method[2] == YES) {
    if (method[0] == YES || method[1] == YES) fprintf(file,", ");
    fprintf(file,"'method_Runge_Kutt_4' title \"Runge-Kutta methods of order 4\" with lines");
  }
  fprintf(file," \npause -1\n"); 
