...
int main(int argc, char ** argv){
  ...
  if (sigaction(SIGCHLD, &sa, 0) < 0){
	...
	if ((key = ftok(argv[0], 'c')) < 0) { perror("ftok"); exit(2); }
	size = nn*sizeof(int);
	...
	for(i = 0; i < nn/2; i++){
		...
		if (pid == 0){
			...
			if ((fd_fifo = open(FIFO, O_RDONLY)) < 0){ perror("open2: "); exit(0); }
			op_sem(sem_id, &r_unlock[0]);
			while (get_sem_val(sem_id, 3) > 0){
				op_sem(sem_id, &r_lock[0]);
				r_arr[0] = 0; r_arr[1] = 0;
				read(fd_fifo, &r_arr, sizeof(int)*2);
