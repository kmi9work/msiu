...
class Controller
  ...
  def auth()
    if !@session['user'].nil? and @session['user'] != ''
      @user = User.find_by_login(@db, @session['user'])
      ...
      if User.check_user(@db, @cgi.params['user'][0], @cgi.params['password'][0])
        @user = User.find_by_login(@db, @cgi.params['user'][0])
  ...
  def menu()
    ...
    unless @user.nil?
      ...
      <hr>
      <ul>
        <li><a href = 'aero.rb?controller=Company&action=list'>Авиакомпании</a></li>
        <li><a href = 'aero.rb?controller=CheckInDesk&action=list'>Регистрационные стойки</a></li>
        <li><a href = 'aero.rb?controller=Terminal&action=list'>Посадочные терминалы</a></li>
      </ul>
      )
    end
    return @menu
