...
class Flight < Model
  attr_accessor :company, :statuses, :terminals, :check_in_desks
...
CREATE TABLE flights(
  ...
  def Flight.find_all(connection, departure = nil)
    ...
    connection.select_all(*query) do |r|
      ...
      f.statuses = FlightStatus.find_all_by_flight_id(connection, f[:id])
      f.terminals = FlightTerminal.find_all_by_flight_id(connection, f[:id])
      f.check_in_desks = FlightCheckInDesk.find_all_by_flight_id(connection, f[:id])
      res << f
    end
    return res
  end
  ...
  def Flight.find_by_id(connection, id)
    ...
    f.company = Company.find_by_id(connection,f[:company_id]) unless f[:company_id].nil?
    f.statuses = FlightStatus.find_all_by_flight_id(connection, id)
    f.terminals = FlightTerminal.find_all_by_flight_id(connection, id)
    f.check_in_desks = FlightCheckInDesk.find_all_by_flight_id(connection, id)
    return f
  end
end
