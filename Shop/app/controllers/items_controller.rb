class ItemsController < ApplicationController
  ...
  def discount_commit
    Discounted.ccreate(Item.find(params[:id]), params['reason'], params['sort'])
    redirect_to(:action => :destroy, :user_name => @user, :id => params[:id])
  end
  ...
  def produce
    begin
      ...
      @item = Discounted.find(params[:id])
    end
  end
  ...
  def produce_commit
    begin
      ...
      i = Discounted.find(params[:id])
    end
    Produce.ccreate(i, params['reason'])
    redirect_to(:action => :destroy, :user_name => @user, :id => params[:id])
  end
  ...
  def destroy
    ...
    if !(i.nil? or i.kind_of?(Discounted))
      id = i.catalogue_id
      i.destroy
      redirect_to(:action => :list, :catalogue_id => id, :user_name => @user)
    elsif i.kind_of?(Discounted)
      i.destroy
      redirect_to(:action => :list, :user_name => @user)
    else
      redirect_to(:action => :list, :user_name => @user)
  ...
  def ItemsController.inline_menu_items(user_class)
    ...
    return [[:show, 'Показать'], [:edit, 'Редактировать'], 
            [:destroy, 'Удалить'], [:list, 'Поставки', :providings], [:produce, "Списать"], [:discount, "Уценить"]]
  end
end
