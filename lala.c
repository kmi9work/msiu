...
=======
  ...
  struct sockaddr_in serv_addr, client_addr, host_addr;
	...
	int nforks = 0;
	char szHostName[255];
>>>>>>> c6a46302bf8fa8469bd0bb7a6fdac5259bbeaa65
  ...
  while(1){
    ...
    if (pid == 0){
      ...
      while(1){
        ...
        while(1){
          bzero(&buff, BUF_SIZE);
          if ((count_r = recv(client_s, &buff, BUF_SIZE, 0)) < 0){
            switch (errno){
            case EINTR : continue;
            }
            perror("recv1");
        ...
        count_r = BUF_SIZE;
        bzero(&buff, BUF_SIZE);
        while ((count_r = recv(host_s, &buff, BUF_SIZE, 0)) == BUF_SIZE){
          fl = 1;
          /*printf("\nFrom host to client=========\n%s\n===========\n", buff);*/
          write(1, buff, count_r);
          printf("%d: Read %d.\n", getpid(), count_r);
          send(client_s, &buff, count_r, 0);
